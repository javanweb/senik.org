import type { FeatureData, TimelineEvent } from './types';

export interface Sublink {
  name: string;
  href: string;
  content?: string;
}

export interface NavLink {
  name: string;
  href: string;
  sublinks?: Sublink[];
  content?: string;
}

export interface CustomThemeColors {
  primary: string;
  primaryHover: string;
  secondary: string;
  background: string;
  cardBackground: string;
  border: string;
  textPrimary: string;
  textSecondary: string;
  textOnPrimary: string;
  heroGradient: string;
}

export interface SiteContent {
  theme: string;
  themeColors?: CustomThemeColors;
  header: {
    navLinks: NavLink[];
  };
  hero: {
    title: string;
    subtitle: string;
    description: string;
    cta_primary: string;
    cta_secondary: string;
    imageUrl: string;
  };
  clients: {
    title: string;
    subtitle: string;
  };
  features: {
    title: string;
    items: FeatureData[];
  };
  softwareShowcase: {
    title: string;
    subtitle: string;
    slides: string[];
  };
  analytics: {
    title1: string;
    paragraph1: string;
    title2: string;
    paragraph2: string;
    cta: string;
    imageUrl: string;
  };
  stats: {
    experience: { value: string; label: string };
    customers: { value: string; label: string };
    team: { value: string; label: string };
    satisfaction: { value: string; label: string };
  };
  moreFeatures: {
    title: string;
    items: FeatureData[];
  };
  about: {
    title: string;
    paragraph: string;
    cta: string;
    timeline: TimelineEvent[];
    imageUrl: string;
  };
  footer: {
    description: string;
    contact: {
      title: string;
      phone: string;
      email: string;
    };
    copyright: string;
  };
}

export const defaultThemeColors: CustomThemeColors = {
  primary: '#009688',
  primaryHover: '#00796B',
  secondary: '#263238',
  background: '#F5F5F5',
  cardBackground: '#ffffff',
  border: '#e5e7eb',
  textPrimary: '#1f2937',
  textSecondary: '#4b5563',
  textOnPrimary: '#ffffff',
  heroGradient: '#E0F2F1',
};

export const siteContent: SiteContent = {
  theme: 'default',
  themeColors: defaultThemeColors,
  header: {
    navLinks: [
      { name: 'صفحه اصلی', href: '/', content: '' },
      { 
        name: 'محصولات', 
        href: '#',
        content: '',
        sublinks: [
            { name: 'نرم افزار فروشگاهی', href: '#', content: '' },
            { name: 'نرم افزار صنعتی', href: '#', content: '' },
            { name: 'نرم افزار رستورانی', href: '#', content: '' },
        ]
      },
      { 
        name: 'خدمات پشتیبانی', 
        href: '#',
        content: '',
        sublinks: [
            { name: 'پشتیبانی آنلاین', href: '#', content: '' },
            { name: 'مرکز دانلود', href: '#', content: '' },
        ]
      },
      { name: 'آموزش', href: '#', content: '' },
      { name: 'آموزشگاه سینتا', href: '#', content: '' },
      { name: 'درباره ما', href: '#', content: '' },
      { name: 'تماس با ما', href: '#', content: '' },
    ]
  },
  hero: {
    title: 'گروه نرم افزاری سه نیک',
    subtitle: 'این نام نیک است که می ماند',
    description: 'امید است ک از تجربه و تخصصمان برای یافتن بهترین نتیجه بهره بگیریم',
    cta_primary: 'همین الان شروع کنید',
    cta_secondary: 'پخش ویدئو',
    imageUrl: 'https://images.unsplash.com/photo-1556740738-b6a63e27c4df?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
  clients: {
    title: 'مشتریان ما',
    subtitle: 'مفتخریم بهترین ها ما را انتخاب می کنند',
  },
  features: {
    title: 'برخی از نرم افزار های سه نیک',
    items: [
      {
        icon: 'data:image/svg+xml;base64,',
        title: 'امکانات نرم افزار فروشگاهی',
        points: [
          'انتقال اطلاعات آفلاین و آنلاین در سطوح مختلف عملیاتی بین شعب صندوق',
          'اتصال به پنل پیامک اینترنتی و ارسال فاکتور و صورت حساب مشتری به صورت پیامک و لینک',
        ],
      },
      {
        icon: 'data:image/svg+xml;base64,',
        title: 'امکانات نرم افزار صنعتی',
        points: [
          'تعریف ساختار حسابها با عمق سه سطح و در هر سطح تا نه رقم کد حساب',
          'تعریف ساختار تفضیلی های شناور درختی مجزا با عمق سی سطح و تا نه رقم کد تفضیلی',
        ],
      },
      {
        icon: 'data:image/svg+xml;base64,',
        title: 'امکانات نرم افزار رستورانی',
        points: [
            'راحت و سریع بودن کار با سفارش گیر ویندوز',
            'قابلیت تعریف فرمول ساخت برای محصولات'
        ]
      },
      {
        icon: 'data:image/svg+xml;base64,',
        title: 'امکانات نرم افزار هتلداری',
        points: [
          'امکان تعریف نامحدود اتاق باظرفیت های مختلف',
          'تعریف انواع مشتری و تعیین نرخ اتاق ها برحسب نوع مشتری',
        ],
      },
    ]
  },
  softwareShowcase: {
    title: 'نمایی از محیط نرم افزار',
    subtitle: 'محیطی کاربرپسند و قدرتمند برای مدیریت یکپارچه امور مالی شما.',
    slides: [
        'https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2070&auto=format&fit=crop',
        'https://images.unsplash.com/photo-1634733620281-9f4c35a6e376?q=80&w=2070&auto=format&fit=crop',
        'https://images.unsplash.com/photo-1611095965942-161b4d3341d3?q=80&w=2070&auto=format&fit=crop',
        'https://images.unsplash.com/photo-1605379399642-870262d3d051?q=80&w=2106&auto=format&fit=crop',
        'https://images.unsplash.com/photo-1633355444132-695d_5876cd00?q=80&w=2070&auto=format&fit=crop',
    ],
  },
  analytics: {
    title1: 'چرا سیستم یکپارچه مالی سه نیک؟',
    paragraph1: 'سه نیک با سال ها تجربه در امور مالی و حسابداری بررسی همه جانبه و عمیقی از خواسته ها و نیازهای بازار کسب کار انجام داده است و تجربه حسابداران و حسابرسان ، مدیران مالی وگردانندگان مشاغل ساده را با دانش مهندسان نرم افزار تلفیق کرده تا به ترکیبی جالب و حرفه ای دست پیدا کند. یکی از نکات مثبت در طراحی سیستم سه نیک این است که طراحان سه نیک علاوه بر دانش برنامه نویسی و نرم افزار دارای تجربه چند ساله در زمینه حسابداری و مشاوره مالی می باشند.',
    title2: 'اهداف سیستم یکپارچه سه نیک',
    paragraph2: 'گروه نرم افزاری سه نیک با تشکیل تیمی تخصصی و متبحر در زمینه های مالی – نرم افزاری و سخت افزاری مفتخر است تا بهترین مشاوره ها در زمینه مکانیزاسیون سیستم های مالی و اداری به شما ارائه نماید. هماهنگی در این حوزه ها(مالی،نرم افزاری،سخت افزاری) برای مشتریان ما این مزیت را به همراه داشته است تا با کمترین هزینه بیشترین خدمات را دریافت نمایند.تیم قدرتمند سه نیک تمامی الزامات و ابزار لازم را برآورد و سپس با موافقت مشتری عملیات اجرا و استقرار سیستم های مالی را در حداقل زمان ممکن مهیا می سازد.',
    cta: 'بیشتر بدانید',
    imageUrl: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2070&auto=format&fit=crop',
  },
  stats: {
     experience: { value: '6', label: 'سال تجربه' },
     customers: { value: '84', label: 'تعداد مشتریان' },
     team: { value: '5', label: 'نفر تیم' },
     satisfaction: { value: '18', label: 'رضایت مشتری' }
  },
  moreFeatures: {
      title: 'برخی از امکانات سه نیک',
      items: [
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم خرید و فروش',
            points: ['امکان ثبت فاکتور فروش با قیمت گذاری های مختلف از قبل تعیین شده', 'امکان ثبت دو نوع سند تولید براساس فرمول تولید و بدون فرمول']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم تولید',
            points: ['تعریف فرمول تولید با دقت بالا', 'امکان ثبت دو نوع سند تولید براساس فرمول تولید و بدون فرمول']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات انبارداری',
            points: ['امکان انبارداری به هر دو روش ادواری و دائمی', 'محاسبه موجودی پایان دوره براساس میانگین موزون و لحظه']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم مالی',
            points: ['معرفی سرفصل حسابها در سطح گروه، کل، معین، تفصیلی به همراه دوسطح شناور مرکز هزینه و مرکز پروژه', 'امکان محاسبه و ثبت سند سود و زیان به صورت اتوماتیک']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم حقوق و دستمزد',
            points: ['ثبت تمامی اطلاعات پرسنل جهت بایگانی و کارگزینی', 'امکان تعریف مزایا و کسورات به تعداد نامحدود به همراه فرمول نویسی']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم ارزی',
            points: ['امکان گزارش صورتحساب ارزی', 'امکان تسعیر ارز و ثبت سند هزینه و درآمدهای حاصل از تسعیر ارزی']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم خزانه داری',
            points: ['امکان چاپ برگه چک پرداختی و فیش واگذاری چک به بانک', 'امکان چاپ چک بر روی برگه چک']
        },
        {
            icon: 'data:image/svg+xml;base64,',
            title: 'امکانات سیستم تسهیلات',
            points: ['امکان ثبت وام های پرداختی شرکت و یادآوری اقساط وام های پرداختی و دریافتی', 'امکان تقسیم سود سال گذشته به سهام داران شرکت براساس معدل حساب سرمایه هر سهامدار']
        }
    ]
  },
  about: {
    title: 'درباره سه نیک',
    paragraph: 'مهندس محمدصادق پاکزاد از سال ۱۳۷۳ در عرصه نرم افزار های حسابداری شروع ب فعالیت کردند و نرم افزار حسابداری امین و آریا را تحت سیستم عامل DOS طراحی و تولید کردند.شرکت آما پردازش ایساتیس نیز در سال 1387 به منظور فعالیت در زمینه تولید نرم افزارهای مالی توسط مهندس محمدصادق پاکزاد تاسیس گردید و راه پر فراز و نشیب رشد و پیشرفت در این عرصه را به منظور کسب تجربه و خدمت به پیشرفت تکنولوژی در کشور با همراهی مدیران و متخصصین علوم فناوری اطلاعات و حسابداری پیموده است. در سال 1401به علت بعضی تغییرات در هیئت مدیره شرکت نام شرکت به آما پردازش فرتاک تغییر کرد و با همان رویکرد روبه جلو ب کار خود ادامه میدهد.',
    cta: 'بیشتر بدانید',
    timeline: [
        { year: '۱۳۷۳', description: 'شروع فعالیت با نرم افزارهای حسابداری امین و آریا تحت سیستم عامل DOS.' },
        { year: '۱۳۸۷', description: 'تاسیس شرکت آما پردازش ایساتیس برای تولید تخصصی نرم افزارهای مالی.' },
        { year: '۱۴۰۱', description: 'تغییر نام به آما پردازش فرتاک و ادامه مسیر رشد با رویکردی نوین.' }
    ],
    imageUrl: 'https://cdn.dribbble.com/users/1787323/screenshots/11310813/media/d83f7214713c79a05b38a7c1ad7071e6.png?resize=1200x900&vertical=center',
  },
  footer: {
    description: 'امید است ک از تجربه و تخصصمان برای یافتن بهترین نتیجه بهره بگیریم',
    contact: {
        title: 'تماس با ما',
        phone: 'شماره همراه: 09925063800',
        email: 'ایمیل: Senikesystem@gmail.com'
    },
    copyright: 'تمامی حقوق مالکیت معنوی این سایت برای شرکت آما پردازش فرتاک یزد محفوظ است.'
  }
};